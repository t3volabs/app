...
async function pushToPasswords(title, username, email, password, TOPT, urls) {
  await db.passwords.add({
    ...
    TOPT: encryptContent(TOPT),
    urls,
    updated_at: new Date().getTime(),
  });
}

const newPassword = ref({ title: "", username: "", email: "", password: "", TOPT: "", urls: "" });
const passwords = ref([]);
...
const loadPasswords = async () => {
  ...
  passwords.value = rawPasswords.map((password) => ({
    ...
    TOPT: password.TOPT ? decryptContent(password.TOPT) : "",
    urls: password.urls,
    visible: false, // Used for toggling password visibility
  }));
};

const addPassword = async () => {
  if (newPassword.value.title && newPassword.value.password) {
    await pushToPasswords(newPassword.value.title, newPassword.value.username, newPassword.value.email, newPassword.value.password, newPassword.value.TOPT, newPassword.value.urls);

    newPassword.value = { title: "", username: "", email: "", password: "", TOPT: "", urls: "" };
    await loadPasswords();
  }
};
...
<template>
  <div class="max-w-2xl mx-auto p-4">
    <div class="mb-6 p-4 border rounded-lg bg-gray-50 shadow">
      ...
      <input v-model="newPassword.TOPT" placeholder="TOTP (Optional)" class="w-full p-2 mb-2 border rounded" />
